...
const timetableAddExtraLectureBtn = document.getElementById('timetableAddExtraLectureBtn');
...
let historyEditingSessionId = null;
let historyEditingStudentIndex = 0;
...
function renderTimetableWeekGrid() {
  ...
  timetableWeekGrid.innerHTML = WEEK_DAYS.map((day) => {
    ...
    if (!dayEntries.length) {
      return `<article class="tt-day-card"><p class="tt-day-title">${day}</p><p class="muted">No lectures</p></article>`;
    }
    ...
    `).join('');
    return `<article class="tt-day-card"><p class="tt-day-title">${day}</p>${rows}</article>`;
  }).join('');
...
function getFilteredHistorySessions() {
  const type = String(historyTypeFilter?.value || 'Theory');
  const classId = String(historyClassFilter?.value || '');
  const date = String(historyDateFilter?.value || '');
  return attendanceSessions
    .filter((session) => session.lectureType === type)
...
function renderHistorySessions() {
  ...
  if (!sessions.length) {
    ...
    historySessionList.innerHTML = 'No saved attendance session found.';
    return;
  }
  ...
  historySessionList.innerHTML = sessions.map((session) => {
    ...
    return `
      <div class="history-item">
        <h4>${session.className} - ${session.subject}</h4>
        <p class="muted">${session.lectureType}${session.batch ? ` | Batch ${session.batch}` : ''} | ${session.date} ${session.startTime}-${session.endTime}</p>
        <p><strong>${present}/${total}</strong> present</p>
        <div class="actions">
  ...
  historySessionList.querySelectorAll('[data-history-edit]').forEach((button) => {
    button.addEventListener('click', () => openHistoryEditor(button.getAttribute('data-history-edit'), 'attendance'));
  });
  historySessionList.querySelectorAll('[data-history-grade]').forEach((button) => {
    button.addEventListener('click', () => openHistoryEditor(button.getAttribute('data-history-grade'), 'grade'));
  });
}

function findHistorySession() {
  return attendanceSessions.find((session) => session.id === historyEditingSessionId) || null;
}

function renderHistoryEditorCard() {
  const session = findHistorySession();
  if (!session) return;
  const record = session.records[historyEditingStudentIndex];
  if (!record) return;
  const percent = computeAttendancePercent(session.classId, record.rollNumber, session.lectureType);
  historyGradeStudentCard.innerHTML = `
    <div class="student-head">
      <span class="student-roll">Roll ${record.rollNumber}</span>
      <strong>${record.name}</strong>
    </div>
    <span class="attendance-percent ${percent >= 75 ? 'good' : 'warn'}">${session.lectureType} Attendance ${percent}%</span>
  `;
...
function openHistoryEditor(sessionId, mode) {
  const session = attendanceSessions.find((item) => item.id === sessionId);
  if (!session) return;
  historyEditingSessionId = sessionId;
  historyEditingStudentIndex = 0;
  historyGradeSearch.value = '';
  if (mode === 'attendance') {
    historyGradeMarksInput.disabled = true;
  }
  renderHistoryEditorCard();
  historyGradeDialog.showModal();
}

function saveHistoryEditorChanges() {
  const session = findHistorySession();
  if (!session) return;
  const record = session.records[historyEditingStudentIndex];
  if (!record) return;
  record.present = historyGradePresentToggle.checked;
  record.statusSource = 'history';

  if (session.lectureType === 'Practical' && session.graded) {
    const value = Number(historyGradeMarksInput.value);
    if (!Number.isFinite(value) || value < 0 || value > Number(session.maxMarks || 0)) {
  ...
  attendanceSessionRepository.save(attendanceSessions);
...
timetableCancelEditBtn?.addEventListener('click', resetTimetableForm);
...
historyLoadBtn?.addEventListener('click', renderHistorySessions);
historyTypeFilter?.addEventListener('change', renderHistorySessions);
historyClassFilter?.addEventListener('change', renderHistorySessions);
historyDateFilter?.addEventListener('change', renderHistorySessions);
historyPrevStudentBtn?.addEventListener('click', () => {
  const session = findHistorySession();
  if (!session) return;
  historyEditingStudentIndex = Math.max(0, historyEditingStudentIndex - 1);
  renderHistoryEditorCard();
});
historyNextStudentBtn?.addEventListener('click', () => {
  const session = findHistorySession();
  if (!session) return;
  historyEditingStudentIndex = Math.min(session.records.length - 1, historyEditingStudentIndex + 1);
  renderHistoryEditorCard();
});
historyGradeSearch?.addEventListener('input', () => {
  ...
  const foundIndex = session.records.findIndex((record) =>
    String(record.rollNumber).includes(query) || String(record.name).toLowerCase().includes(query)
  );
  if (foundIndex >= 0) {
    historyEditingStudentIndex = foundIndex;
    renderHistoryEditorCard();
  }
});
historyGradePresentToggle?.addEventListener('change', () => {
  if (!historyGradePresentToggle.checked) {
    historyGradeMarksInput.value = '0';
    historyGradeMarksInput.classList.add('warn');
  } else {
    historyGradeMarksInput.classList.remove('warn');
  }
...
historyGradeMarksInput?.addEventListener('keydown', (event) => {
  ...
  saveHistoryEditorChanges();
  const session = findHistorySession();
  if (!session) return;
  historyEditingStudentIndex = Math.min(session.records.length - 1, historyEditingStudentIndex + 1);
  renderHistoryEditorCard();
});
historyGradeSaveBtn?.addEventListener('click', saveHistoryEditorChanges);
dashboardAddTimetableBtn?.addEventListener('click', () => {
  switchView('timetable');
  resetTimetableForm();
  timetableClassSubjectSelect?.focus();
});
timetableAddExtraLectureBtn?.addEventListener('click', openExtraLectureFlow);
...
renderLectures();
populateHistoryClassOptions();
if (historyDateFilter) historyDateFilter.value = getDateKey();
updateAttendanceClassOptions();
